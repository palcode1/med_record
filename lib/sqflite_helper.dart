...
class SqfliteHelper {
  ...
  Future<Database> openMyDatabase() async {
    return await openDatabase(
      ...
      join(await getDatabasesPath(), 'medrecord.db'),
      ...
      onCreate: (db, version) {
        return db.execute(
          'CREATE TABLE medRecord(id INTEGER PRIMARY KEY AUTOINCREMENT NOT NULL, title TEXT, description TEXT, status  INTEGER)',
        );
  ...
  Future<void> insertTask(String title, String description, bool status) async {
  ...
  Future<void> deleteTask(int id) async {
    ...
    db.delete('medRecord', where: 'id = ?', whereArgs: [id]);
  }
  ...
  Future<List<Map<String, dynamic>>> getTasks() async {
    ...
    return await db.query('medRecord');
  }
}
